<?php
    ...
    $sqlSeleccionar = "SELECT * FROM Familia WHERE ID = $ID_u";
...
<body>
    <div class = "contenedor_principal">
        ...
        <div class = "segundo">
            <div class="cabecera">
                <h1 class="titulo">Editar familia</h1>
            </div>
            <form method="POST" name = "formulario" class = "col-4 p-3 m-auto">
                <?php while ($fila = $registros->fetch_array()) { ?>
                    ...
                    <div class="mb-1">
                        <label for="txtFamilia" class="form-label">Nombre de la familia: </label>
                        <input type="text" value="<?= $fila[1]; ?>" class="form-control" id="txtFamilia" name="txtFamilia" placeholder="Familia..." required>
                    </div>
    ...
    <?php
        if(isset($_POST['btnActualizar'])){
            if(!empty($_POST['txtFamilia'])){
                $ID = $_POST['id'];
                $Fam = $_POST['txtFamilia'];

                $sql_actualizar = "UPDATE Familia SET Familia = '$Fam' WHERE ID = $ID";
